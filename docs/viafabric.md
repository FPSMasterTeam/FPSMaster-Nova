...
`xvfb-run ./gradlew :1.21.11:runClient -PwithViaFabric` loaded `viafabric 0.4.21+181-1.14-1.21`, `viafabric-mc12111`, and `viaversion 5.12.0-SNAPSHOT`. ViaVersion finished mapping loading; FPSMaster initialized; LWJGL started; resource reload included viafabric + viaversion. The previous `MixinRegistrySyncManager` apply crash, `class_1132` classload crash, and jvmdg `Runtime.Version.feature` crash did not recur.

`xvfb-run ./gradlew :1.21.11:runClient -PwithViaFabricPlus` loaded `viafabricplus 4.4.15` and `viafabricplus-visuals 4.4.15` (52 mods). The previous exclusive `@Redirect` crash (`MixinLivingEntity.forceSideMove` vs Plus `alwaysRotateWhenWalkingBackwards` on `LivingEntity.tick` → `Mth.abs`) did not recur. ViaVersion finished mapping loading; FPSMaster initialized; resource reload included viafabricplus + visuals; atlases created. Headless OpenAL/ALSA failed in this environment (no sound device) and is unrelated. 26.2 was not launched here (needs JDK 25).
