...
object CosmeticView {
    ...
    fun rendersDragonWings(entityId: Int): Boolean = if (isLocal(entityId)) {
        ...
        uuidOf(entityId)?.let(CosmeticLoadoutCache::rendersDragonWings) == true
    }
    ...
    fun rendersElytra(entityId: Int): Boolean = if (isLocal(entityId)) {
        ...
        uuidOf(entityId)?.let(CosmeticLoadoutCache::rendersElytra) == true
    }
    ...
    fun wingTexture(entityId: Int): TextureId? = if (isLocal(entityId)) {
        ...
        uuidOf(entityId)?.let(CosmeticLoadoutCache::wingTexture)
    }
    ...
    fun capeTexture(entityId: Int): TextureId? = if (isLocal(entityId)) {
        ...
        uuidOf(entityId)?.let(CosmeticLoadoutCache::capeTexture)
    }
    ...
    fun wingScale(entityId: Int): Float = if (isLocal(entityId)) {
        ...
        uuidOf(entityId)?.let(CosmeticLoadoutCache::wingScale) ?: 1f
    }
    ...
    fun animatesCape(entityId: Int): Boolean = if (isLocal(entityId)) {
        ...
        uuidOf(entityId)?.let(CosmeticLoadoutCache::animatesCape) == true
    }
